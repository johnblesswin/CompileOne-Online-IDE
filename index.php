...
$codeoj_data = "nodata";
$codeoj_ext = "cpp";
$codeoj_access = "private";

if(isset($_POST['operation']) && $_POST['operation'] == 'code_run') {
	// $fs /var/www/test/aqfaridi/
	$fs = new fs(dirname(__FILE__) . DIRECTORY_SEPARATOR . 'users' . DIRECTORY_SEPARATOR . 'guest' . DIRECTORY_SEPARATOR);
	try {
		$rslt = null;
...
<html>
	...
	<head>
		...
		<meta name="Description" content="Compile and Execute Programs Online  - You can compile and run, execute your sourc
e code in more than 15 programming languages like Java, C, C++, Pascal,PHP, Perl, Ruby and Python,Haskell,Pike programs online using
 your browser." />
		<meta name="Keywords" content="Unix, Web, Compile, Run, Java, C, C++, PHP, Perl, Ruby,Haskell,Pike and Python Progra
ms" />
...
<?php
if(isset($_POST['operation']) && $_POST['operation']=='codeoj') {
	try {

		global $codeoj_data,$codeoj_ext,$codeoj_access;
		$codeoj_access = $_POST['access'];
		$codeoj_data = $_POST['source'];
		$codeoj_ext = $_POST['ext'];	
...
<div class="modal fade" id="stdoutshow" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <textarea id="stdout1" style="font-weight:bold; width:700px; resize:none;" wrap="off" readonly rows="15" value= ></textarea>
      </div>
      <div class="modal-footer">
...
var mp = {};
	...
	<script>
		...
		$( document ).ready(function() {
			 ...
			 setTimeout(function(){
				save_data = <?php echo json_encode($codeoj_data);?>;
				var ext = <?php echo json_encode($codeoj_ext);?>;
				var access = <?php echo json_encode($codeoj_access);?>;
				if(save_data == "nodata")
				{
					$('#langSelect').val('cpp');
					$.get('?operation=temp_node', {'ext' : 'cpp'},function(dt){window.editor.getSession().setValue(dt);});
				}
				else
				{
					$('#langSelect').val(ext);
					window.editor.getSession().setValue(save_data); 
				}
	 ...
	 resizeAce();
	   ...
	   $("#keybinding li").click(function( event ) {
			...
			$("#btn-export").click(function(){
				...
				{ 
					var extension = $("#langSelect option:selected").val();
			...
			$("#btn-run").click(function(){
					...
					{	
						...
						setTimeout(function () {  $.post('index.php',{'operation' : 'code_run', 'tl':tl, 'lang':lang,'access':'public','stdin':input,'sdata':save_data,'fname':filename,'ext':extension},function(dt){
